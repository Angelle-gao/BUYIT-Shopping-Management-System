...
namespace ShoppingSystem20201223
{
    public partial class order_process : Form
    {
        ...
        private void order_process_Load(object sender, EventArgs e)
        {
            // TODO: 这行代码将数据加载到表“gOODSDataSet7.user_order”中。您可以根据需要移动或删除它。
            this.user_orderTableAdapter3.Fill(this.gOODSDataSet7.user_order);
            this.userorderBindingSource.Filter = "订单编号 like '%" + this.textBox1.Text.Trim() + "%'";
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand comm = new SqlCommand("select * from user_order", conn);//读取user_order中的全部数据
            da = new SqlDataAdapter(comm);
            SqlCommandBuilder builder = new SqlCommandBuilder(da);
            ds = new DataSet();
            da.Fill(ds);
            this.dataGridView1.DataSource = ds.Tables[0];//显示数据为user_order中的数据
        }
