...
{
    public partial class history_order : Form
    {
        ...
        SqlDataAdapter da;
        DataSet ds;
        private void history_order_Load(object sender, EventArgs e)
        {
            this.dataGridView1.ReadOnly = true;//设置数据为只读（不允许修改）
            string strConn = ConfigurationManager.AppSettings["ConnStr"].ToString();
            SqlConnection conn = new SqlConnection(strConn);
            SqlCommand comm = new SqlCommand("select * from user_order where 订单状态=4 or 订单状态=2", conn);//读取user_order表
            da = new SqlDataAdapter(comm);
            SqlCommandBuilder builder = new SqlCommandBuilder(da);
            ds = new DataSet();
            da.Fill(ds);
            this.dataGridView1.DataSource = ds.Tables[0];//显示数据为用户订单表中订单状态的记录
        }
